...
import { getRecentAgentActivityCount } from "@/lib/agent-pipeline";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [{ count }, aiActivity] = await Promise.all([
    supabase.from("notifications").select("*", { count: "exact", head: true }).eq("read", false),
    getRecentAgentActivityCount(profile),
  ]);
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="flex-1 min-w-0 flex flex-col px-2 py-2 md:pl-0">
        <div className="flex-1 min-h-0 flex flex-col overflow-hidden rounded-2xl bg-card ring-1 ring-foreground/[0.06] shadow-card">
          <AppHeader profile={profile} unread={count ?? 0} aiActivity={aiActivity} />
